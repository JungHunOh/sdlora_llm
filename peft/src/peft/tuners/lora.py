...
class Linear(nn.Linear, LoraLayer):
    ...
    def train(self, mode: bool = True):
        ...
        if not mode and self.merge_weights and not self.merged:
            ...
            if self.r > 0:
                if self.sign_preserve:
                    self.weight.data = abs(self.weight + transpose(self.lora_B.weight @ self.lora_A.weight, self.fan_in_fan_out) * self.scaling) * torch.sign(self.weight)
                else:
                    self.weight.data += (
            ...
            if self.r > 0:
                if self.sign_preserve:
                    if self.changed_signs is None:
                        self.weight.data = abs(self.weight - transpose(self.lora_B.weight @ self.lora_A.weight, self.fan_in_fan_out) * self.scaling) * torch.sign(self.weight)
                    else:
                        self.weight.data = abs(self.changed_signs * self.weight - transpose(self.lora_B.weight @ self.lora_A.weight, self.fan_in_fan_out) * self.scaling) * torch.sign(self.weight)
                else:
                    self.weight.data -= (
                        transpose(self.lora_B.weight @ self.lora_A.weight, self.fan_in_fan_out) * self.scaling
                    )
    ...
    def forward(self, x: torch.Tensor):
        ...
        if self.disable_adapters:
            if self.r > 0 and self.merged:
                ...
                if self.sign_preserve:
                    self.weight.data = abs(self.changed_signs * self.weight - transpose(matmul_output.to(previous_dtype), self.fan_in_fan_out) * self.scaling) * torch.sign(self.weight.data)
                else:
            ...
            if self.r > 0:
                if self.sign_preserve:
                    ...
                    result = F.linear(x, transpose(self.weight, self.fan_in_fan_out), bias=self.bias) + self.lora_B(self.lora_A(self.lora_dropout(x.to(self.lora_A.weight.dtype)))) * self.scaling
